...
dist = zeros(n_states,2);
in_track = zeros(n_states,1);

for i = 1:n_states 
    
   inBound_k    =  [ innerBoundary(i,1,1)  innerBoundary(i,2,1)]; 
   outBound_k    = [ outerBoundary(i,1,1)  outerBoundary(i,2,1)];
   
   if( i ~= n_states )
       inBound_kp1    =  [ innerBoundary(i+1,1,1)  innerBoundary(i+1,2,1) ]; 
       outBound_kp1   =  [ outerBoundary(i+1,1,1)  outerBoundary(i+1,2,1) ];
   else
       inBound_kp1    =  [ innerBoundary(1,1,1)  innerBoundary(1,2,1) ]; 
       outBound_kp1   =  [ outerBoundary(1,1,1)  outerBoundary(1,2,1) ];
   end
   
   dist(i,1) = point_to_line_distance(xi(1:2,i)',inBound_k,inBound_kp1);
   dist(i,2) = point_to_line_distance(xi(1:2,i)',outBound_k,outBound_kp1);
    
   in_track(i) = road_width - (dist(i,1) + dist(i,2));
