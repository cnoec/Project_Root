...
extern void trajectory_simulation_derivatives(real_T arg_steer_input, real_T
  arg_theta[13], real_T arg_speed_ref, real_T arg_xi_0[6]);
...
static void rt_ertODEUpdateContinuousStates(RTWSolverInfo *si , real_T
  arg_steer_input, real_T arg_theta[13], real_T arg_speed_ref, real_T arg_xi_0[6])
{
  ...
  /* f0 = f(t,y) */
  rtsiSetdX(si, f0);
  trajectory_simulation_derivatives(arg_steer_input, arg_theta, arg_speed_ref,
    arg_xi_0);
  ...
  rtsiSetT(si, t + temp);
  rtsiSetdX(si, f1);
  trajectory_simulation_step(arg_steer_input, arg_theta, arg_speed_ref, arg_xi_0);
  trajectory_simulation_derivatives(arg_steer_input, arg_theta, arg_speed_ref,
    arg_xi_0);
...
/* Model step function */
void trajectory_simulation_step(real_T arg_steer_input, real_T arg_theta[13],
  real_T arg_speed_ref, real_T arg_xi_0[6])
{
  real_T Uy;
  real_T F_z_f;
  real_T F_z_r;
  ...
  if (rtDW.Integrator_IWORK != 0) {
    for (i = 0; i < 6; i++) {
      rtX.Integrator_CSTATE[i] = arg_xi_0[i];
    }
  }
   ...
   */
  rtDW.Sum1 = arg_speed_ref - rtX.Integrator_CSTATE[2];
  if (rtmIsMajorTimeStep(rtM)) {
  ...
  F_z_f_tmp = arg_theta[2] + arg_theta[3];
  F_z_f = arg_theta[0] * 9.81 * arg_theta[3] / F_z_f_tmp;
  F_z_r = arg_theta[0] * 9.81 * arg_theta[2] / F_z_f_tmp;
  z_f = tan(rt_atan2d_snf(arg_theta[2] * rtX.Integrator_CSTATE[5] + Uy,
             rtX.Integrator_CSTATE[2]) - arg_steer_input);
  z_r = tan(rt_atan2d_snf(Uy - arg_theta[3] * rtX.Integrator_CSTATE[5],
             rtX.Integrator_CSTATE[2]));
  F_z_f_tmp = arg_theta[7] * arg_theta[7] * 27.0;
  F_z_f = fmin(arg_theta[7] * F_z_f, fmax(-arg_theta[7] * F_z_f, (arg_theta[4] *
    arg_theta[4] * fabs(z_f) * z_f / (3.0 * arg_theta[7] * F_z_f) + -arg_theta[4]
    * z_f) - rt_powd_snf(arg_theta[4], 3.0) * rt_powd_snf(z_f, 3.0) / (F_z_f_tmp
    * (F_z_f * F_z_f))));
  F_z_r = fmin(arg_theta[7] * F_z_r, fmax(-arg_theta[7] * F_z_r, (arg_theta[5] *
    arg_theta[5] * fabs(z_r) * z_r / (3.0 * arg_theta[7] * F_z_r) + -arg_theta[5]
    * z_r) - rt_powd_snf(arg_theta[5], 3.0) * rt_powd_snf(z_r, 3.0) / (F_z_f_tmp
    * (F_z_r * F_z_r))));
   ...
   *  Integrator: '<Root>/Integrator'
   */
  rtDW.xi_dot[2] = (((u0 / arg_theta[6] - F_z_f * sin(arg_steer_input)) -
                     arg_theta[11] * rtX.Integrator_CSTATE[2]) - 0.5 *
                    arg_theta[12] * arg_theta[8] * arg_theta[10] *
                    (rtX.Integrator_CSTATE[2] * rtX.Integrator_CSTATE[2])) *
    (1.0 / arg_theta[0]);
  F_z_f_tmp = cos(arg_steer_input);
  rtDW.xi_dot[3] = 1.0 / (arg_theta[0] * rtX.Integrator_CSTATE[2]) * (F_z_f *
    F_z_f_tmp + F_z_r) - rtX.Integrator_CSTATE[5];
  rtDW.xi_dot[4] = rtX.Integrator_CSTATE[5];
  rtDW.xi_dot[5] = (arg_theta[2] * F_z_f * F_z_f_tmp - arg_theta[3] * F_z_r) *
    (1.0 / arg_theta[1]);
  if (rtmIsMajorTimeStep(rtM)) {
  ...
  if (rtmIsMajorTimeStep(rtM)) {
    rt_ertODEUpdateContinuousStates(&rtM->solverInfo, arg_steer_input, arg_theta,
      arg_speed_ref, arg_xi_0);
...
void trajectory_simulation_derivatives(real_T arg_steer_input, real_T arg_theta
...
void trajectory_simulation_initializer(void)
{
  /* Registration code */
